  ...
  $("#nav-notifications").on("click", function (e) {
    ...
    $.get("/get-notifications", {}, (data, status, xhr) => {
      data.forEach((nt) => {
        ...
        let notif_text_container = new Element(
        ...
        let read_indicator;
        if (nt.isRead) {
          read_indicator = new Element("i.fa.fa-circle.read", {}).getElement();
        } else {
          read_indicator = new Element("i.fa.fa-circle", {}).getElement();
        }
...
function onNotifClick(e) {
  ...
  document.querySelector(
    "#modal-reservation-details-container-notif"
  ).innerHTML = "";
  let notif_id = e.currentTarget
    .closest(".notif-details-container")
    .getAttribute("data-notif-id");
  $.get("/find-notification", { id: notif_id }, (data, status, xhr) => {
    if (
      data.notif_details.type == "Admin Set Pending" ||
      data.notif_details.type == "Admin Set Approved" ||
      data.notif_details.type == "Admin Set Cancelled" ||
      data.notif_details.type == "Reservation Pending" ||
      data.notif_details.type == "Employee Set Cancelled" ||
      data.notif_details.type == "Employee Set Pending" ||
      data.notif_details.type == "Employee Set Approved" ||
      data.notif_details.type == "Customer Set Cancelled"
    ) {
      document.querySelector("#modal-notif-title").innerText =
        data.notif_details.title;
      document.querySelector("#modal-notif-body").innerText =
        data.notif_details.body;
      document.querySelector("#modal-notif-timestamp").innerText = dayjs(
        data.notif_details.timestamp
      ).format("MMMM DD, YYYY, hh:mm A");
      document.querySelector(
        "#modal-reservation-details-container-notif"
      ).style.display = "block";
      let reservation_details_container = document.querySelector(
        "#modal-reservation-details-container-notif"
      );
      ...
      let services_container = new Element(
        ".services-container-notif"
      ).getElement();
      let time_formatted = dayjs(data.reservation_details.timestamp).format(
        "MMM DD, YYYY hh:mm A"
      );
      ...
      data.reservation_details.services.forEach((srv) => {
        ...
        let preferred_employee = new Element(
          ".preferred-employee.detail-notif",
          {
      ...
      reservation_details_container.append(
        reason_label,
        timestamp,
        services_label,
        services_container
