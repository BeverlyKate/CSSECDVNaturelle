...
function doesAddServiceCollHaveEmptyField() {
  let doesTabularHaveEmpty = $("#add-tabular-services-list li input").filter(
    function () {
      return $(this).val() === "";
    }
  );

  let doesStandaloneHaveEmpty = $(
    "#add-standalone-services-list li input"
  ).filter(function () {
    return $(this).val() === "";
  });
...
function doesEditServiceCollHaveEmptyField() {
  let doesTabularHaveEmpty = $("#edit-tabular-services-list li input").filter(
    function () {
      return $(this).val() === "";
    }
  );
  ...
  let doesStandaloneHaveEmpty = $(
    "#edit-standalone-services-list li input"
  ).filter(function () {
    return $(this).val() === "";
  });
...
$(document).ready(function () {
  showServiceCollections(
    SERVICE_COLLECTION_GET_URL,
    SERVICE_COLLECTION_WRAPPER
  );
  ...
  $("#add-service-collection-btn").on("click", function (e) {
    ...
    if (!service_concern) {
      ...
      showError(
        ...
        "#add-service-collection-error-msg"
      );
      ...
      let optionChoices1 = getUniqueValues(
        tabular_services_arr,
        "serviceOption1"
      );
      let optionChoices2 = getUniqueValues(
        tabular_services_arr,
        "serviceOption2"
      );
      ...
      $.post(
        ...
        function (response) {
          if (response.hasError) {
            ...
            add_form[0].reset();
            showSuccess(
              "Added Service Collection successfully!",
              "#add-service-collection-error-msg"
            );
  ...
  $("#delete-service-collection-btn").on("click", function (e) {
    e.preventDefault();
    let serviceTitle = {
      serviceTitle: $(".service-title-info").first().text(),
    };
    ...
    $.post(
      ...
      function (response) {
        if (response.hasError) {
          ...
          e.preventDefault();
          showSuccess(
            "Deleted " + serviceTitle.serviceTitle + " successfully!",
            "#delete-service-collection-error-msg"
          );
          $("#service-collection-container").empty();
          showServiceCollections(
            SERVICE_COLLECTION_GET_URL,
    ...
    .forEach((modal) => {
      modal.addEventListener("hidden.bs.modal", function () {
        ...
        document.querySelector(
          "#add-tabular-services-list"
        ).innerHTML = `<li class="input-tabular-services">
                                                                        <input class="form-control input-tabular-service-option-1" name="input-tabular-service-option-1" type="text" placeholder="Service Option 1">
                                                                        <input class="form-control input-tabular-service-option-2" name="input-tabular-service-option-2" type="text" placeholder="Service Option 2">
        ...
        document.querySelector(
          "#add-standalone-services-list"
        ).innerHTML = `<li class="input-standalone-services">
                                                                                    <input class="form-control input-standalone-service-option" name="input-tabular-service-option-1" type="text" placeholder="Service Option">
...
function showServiceCollections(url, container) {
  $.get(url, {}, (data, status, xhr) => {
    data.forEach((sc) => {
      let service_collection_preview_container = new Element(
      ...
      let service_collection_info_container = new Element(
      ...
      let service_option_tags_container = new Element(
        ".service-option-tags-container"
      ).getElement();
      let option1_tags_container = new Element(
        ".option1-tags-container"
      ).getElement();
      let option2_tags_container = new Element(
        ".option2-tags-container"
      ).getElement();
      ...
      service_option_tags_container.append(
        option1_tags_container,
        option2_tags_container
      );

      service_collection_info_container.append(
        service_title,
        service_option_tags_container
      );

      let service_collection_controls_container = new Element(
      ...
      service_collection_preview_container.append(
        service_collection_info_container,
        service_collection_controls_container
      );

      document
        .querySelector(container)
        .append(service_collection_preview_container);
    });
  });
}
...
function onBtnEditClick(e) {
  let service_collection_id = e.currentTarget
    .closest(".service-collection-preview-container")
    .getAttribute("data-service-collection-id");
  let modal_edit = document.getElementById("edit-service-collection-modal");
  let modal_edit_id = modal_edit.querySelector(
    "#edit-input-service-collections-id"
  );
  ...
  $.get(
    ...
    { id: service_collection_id },
    (data, status, xhr) => {
      document.querySelector("#edit-input-service-concern").value =
        data.serviceConcern;
      document.querySelector("#edit-input-service-title").value =
        data.serviceTitle;

      let tabular_services_list = document.querySelector(
        "#edit-tabular-services-list"
      );
      let standalone_services_list = document.querySelector(
        "#edit-standalone-services-list"
      );
      ...
      if (Array.isArray(data.services) && data.services.length !== 0) {
        data.services.forEach((service) => {
          let input_tabular_services = new Element(
            "li.input-tabular-services"
          ).getElement();
          let input_tabular_service_option_1 = new Element(
            "input.form-control.input-tabular-service-option-1",
            {
              attr: {
                type: "text",
          ...
          let input_tabular_service_option_2 = new Element(
            "input.form-control.input-tabular-service-option-2",
            {
              attr: {
                type: "text",
          ...
          let input_price = new Element(
            "input.form-control.input-tabular-price",
            {
              attr: {
                type: "number",
                placeholder: "Price",
          ...
          let delete_tabular_service_button = new Element(
            "button.delete-tabular-service"
          ).getElement();
          let delete_service_icon = new Element(
            "i.fa.fa-trash-can"
          ).getElement();
          delete_tabular_service_button.append(delete_service_icon);

          input_tabular_services.append(
            input_tabular_service_option_1,
            input_tabular_service_option_2,
            input_price,
            delete_tabular_service_button
          );
      ...
      if (
        Array.isArray(data.specialServices) &&
        data.specialServices.length !== 0
      ) {
        data.specialServices.forEach((service) => {
          let input_standalone_services = new Element(
            "li.input-standalone-services"
          ).getElement();
          let input_standalone_service_option = new Element(
            "input.form-control.input-standalone-service-option",
            {
              attr: {
                type: "text",
          ...
          let input_standalone_price = new Element(
            "input.form-control.input-standalone-price",
            {
              attr: {
                type: "number",
                placeholder: "Price",
          ...
          let delete_standalone_service_button = new Element(
            "button.delete-standalone-service"
          ).getElement();
          let delete_service_icon = new Element(
            "i.fa.fa-trash-can"
          ).getElement();
          ...
          input_standalone_services.append(
            input_standalone_service_option,
            input_standalone_price,
            delete_standalone_service_button
          );
