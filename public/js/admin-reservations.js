...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("btn-modal-reservation-save")
    .addEventListener("click", function (e) {
      let modal_reservation_status = this.closest("#modal-reservation-status");

      let reservation_id = document.getElementById(
        "input-reservation-id"
      ).value;
      let reservation_status_radio = document.querySelector(
        "#modal-reservation-status-controls-container > input[type='radio']:checked"
      );
      let reason_for_change = document.getElementById(
      ...
      reservation_status =
        reservation_status.charAt(0).toUpperCase() +
        reservation_status.slice(1);

      let btn_save =
        this.closest(".modal-content").querySelector(".btn-modal-success");
  ...
  document.querySelectorAll("#modal-reservation-status").forEach((modal) => {
    modal.addEventListener("hidden.bs.modal", function () {
      ...
      this.querySelectorAll("input[type=checkbox], input[type=radio]").forEach(
        (input) => {
          input.checked = "";
        }
      );
...
function resetModalServices() {
  let modal_reservation_services = document.querySelector(
    "#modal-reservation-services"
  );

  let modal_reservation_services_container =
    modal_reservation_services.querySelector(
      "#modal-reservation-services-container"
    );
...
function onBtnServicesClick(e) {
  ...
  let reservation_id = e.currentTarget
    .closest(".reservation-container")
    .getAttribute("data-reservation-id");

  let reservation_user = e.currentTarget
    .closest(".reservation-container")
    .querySelector(".reservation-user").textContent;
  let reservation_datetime = e.currentTarget
    .closest(".reservation-container")
    .querySelector(".reservation-datetime").textContent;

  let modal_reservation_user = document.getElementById(
    "modal-reservation-services-user"
  );
  let modal_reservation_datetime = document.getElementById(
    "modal-reservation-services-datetime"
  );
  ...
  $.get(
    ...
    (data, status, xhr) => {
      ...
      for (var i = 0; i < data.services.length; i++) {
        ...
        let accordion_button = new Element(
        ...
        document
          .querySelector("#modal-reservation-services-container")
          .append(accordion_item);
      }

      bootstrap.Modal.getOrCreateInstance(
        document.querySelector("#modal-reservation-services")
      ).show();
    }
  );
}
...
function resetModalStatus() {
  let modal_reservation_status = document.querySelector(
    "#modal-reservation-status"
  );

  modal_reservation_status
    .querySelectorAll(
      "#modal-reservation-status-controls-container > input[type='radio']"
    )
    .forEach((i) => {
      i.checked = false;
      i.disabled = false;
  ...
  modal_reservation_status
    .querySelectorAll(
      "#modal-reservation-status-controls-container > label.btn"
    )
    .forEach((lb) => {
...
function onBtnUpdateClick(e) {
  ...
  let modal_reservation_user = document.getElementById(
    "modal-reservation-status-user"
  );
  let modal_reservation_datetime = document.getElementById(
    "modal-reservation-status-datetime"
  );

  modal_reservation_user.textContent = reservation_user;
  modal_reservation_datetime.textContent = reservation_datetime;

  let reservation_status = e.currentTarget
    .closest(".reservation-container")
    .querySelector(".reservation-status")
    .getAttribute("data-reservation-status");
  let modal_reservation_status_radio = document.getElementById(
    "status-" + reservation_status
  );
  modal_reservation_status_radio.checked = true;
  modal_reservation_status_radio.disabled = true;
  let modal_reservation_status_btn = document.querySelector(
    ".btn.status-" + reservation_status
  );
...
function showReservations(url, container) {
  $.get(url, {}, (data, status, xhr) => {
    ...
    data.forEach((reservation) => {
      ...
      let reservation_details = new Element(
        ".reservation-details"
      ).getElement();
      ...
      let status_icon = new Element(
        "i.fa." + status_icon_class.get(reservation.status)
      ).getElement();
      reservation_status.prepend(status_icon);

      reservation_header.append(reservation_details, reservation_status);

      let reservation_controls = new Element(
        ".reservation-controls"
      ).getElement();

      let btn_services = new Element(
        "button.btn.admin-btn-reservation-services",
        {
          text: "Services",
        }
      ).getElement();
