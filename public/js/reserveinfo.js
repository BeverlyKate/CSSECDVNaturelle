  ...
  $.get(url, {}, (data, status, xhr) => {
    data.forEach((rsrv) => {
      ...
      if (rsrv.status == "Cancelled") {
        reservation_details_container = new Element(
          ".reservation-details-container.cancelled",
          {
            attr: {
      ...
      let cancel_reservation_btn = new Element(
      ...
      cancel_reservation_btn.addEventListener(
        "click",
...
function onCancelReservationClick(e) {
  let reservation_id = e.currentTarget
    .closest(".reservation-details-container")
  ...
  let cancel_services_container = document.getElementById(
    "cancel-services-container"
  );
  modal_confirm_id.value = reservation_id;
  $.get(
    "/reserveinfo/find-reservation",
    { id: reservation_id },
    (data, status, xhr) => {
      document.querySelector("#input-reservation-date").value = data.timestamp;

      data.services.forEach((serv) => {
        let confirm_services_container = new Element(
          ".confirm-services-container"
        ).getElement();
        let label_service_title = new Element("div.desc", {
          text: "Service Title: ",
        }).getElement();
        ...
        let confirm_service_title = new Element(
          ".confirm-service-title.detail",
          {
            text: serv.serviceTitle,
          }
        ).getElement();
        let confirm_preferred_employee = new Element(
          ".confirm-preferred-employee.detail",
          {
        ...
        if (serv.details != "") {
          ...
          confirm_services_container.append(
            label_service_title,
            label_preferred_employee,
            label_details
          );
